...
/** 
  @Function
    void WS2812_initializeLedArray(ws2812_t leds[], int arrayLength)

  @Summary
    Setea todos los LEDs apagados.

  @Description
    Setea todos los LEDs apagados.
    <p>
    Se recorren todos los LEDs introduciendo el valor R=0, G=0 y B=0.
    <p>
  ...
  @Parameters
    @param leds[] Valores RGB de todos los LED almacenados en un array.
 */
void WS2812_initializeLedArray(ws2812_t leds[], int arrayLength) {
    int i;
    ...
    WS2812_send(leds, arrayLength);
}

/** 
  @Function
    void WS2812_setLEDColor(ws2812_t leds[], int ledColor, int ledNumber)

  @Summary
    Setea el valor RGB de un LED determinado.
  ...
  @Parameters
    @param leds[] Valores RGB de todos los LED almacenados en un array.
    @param ledColor Color a setear.
    @param ledNumber LED al cual se le cambiara el color.
 */
void WS2812_setLEDColor(ws2812_t leds[], int ledColor, int ledNumber) {
    switch (ledColor) {
        case 0:
            leds[ledNumber] = WHITE;
            break;
        case 1:
            leds[ledNumber] = RED;
            break;
